...
def graph_vectors(v1, v2, start_point, function):
    """ Graphs the functions that take two vectors and return an output. """
    fig = plt.figure()
    axes = fig.add_subplot(111, projection = '3d')      # used to get 3D axes
    v3 = function(v1, v2)
    x_starts, y_starts, z_starts = zip(start_point, start_point, start_point)   # used to set vector start points
    x_coords, y_coords, z_coords = zip(v1, v2, v3)      # used to set vector lengths
    print ("x_coords are", x_coords, "y_coords are", y_coords, "z_coords are", z_coords)

    # scales the axes according to the vectors shown
...
def graph_lincomb(vector_list, scalar_list, start_point):
    """ Graphs the linear combination of a list of vectors and scalars. """
    fig = plt.figure()
...
def graph_scaled(vector, scalar, start_point):
    """ Graphs a vector scaled by a scalar"""
    fig = plt.figure()
    axes = fig.add_subplot(111, projection = '3d')
...
# arbitrary data values initialized for testing the functions in the custom module
