...
// ReadSelection reads the dimension given the optionName.
//   returns the character to be used in draw.
//   returns the number of characters needed to draw Figure in dimension
char ReadSelection (string optionName,int & dimension);

// class Figure
//      Abstract class knows how to Move and Place Figures. Contains the
//      position of a Figure, but doesn't know how to Display or Edit Figures.
//
//      The pure virtual functions must be implemented in the derived classes:
//      eg. Rectangle, Triangle.
//
class Figure
{
	public:
		...
		virtual void Edit()=0; 
		// This function is virtual, meaning that compiler needs to defer the binding
		// to runtime, i.e., it waits until running time to decide which version of 
...
// class Triangle
//   Derived from parent Figure. It knows how to Display and Edit a Rectangle.
//   It implements the pure virtual functions that were declared in the abstract
//   Figure class.
class Triangle: public Figure
{
	public:
		// x_o and y_o are the initial x and y coordinates of the shape
		// h is the height of the triangle
		// symbol is the symbol used to make the triangle
		Triangle (int x_o, int y_o, int h=2, char symbol='*'):Figure(x_o,y_o)
		{
			height = 2;
			c = symbol;
		}
		...
		// Edit()
		// Rereads character and width of Rectangle
		void Edit()
		{
			// used as dummy argument for ReadSelection
			int placeholder;

			// read character
			c = ReadSelection ("Character", placeholder);
			...
			// EDIT
			// Display(bool erase)
			// Displays the Triangle using the character and width.
			void Display (bool erase=false)
   			{
				...
				for (int heightCounter = 0; heightCounter < height; heightCounter++)
				{
					...
					{
						str[widthCounter] = (erase? ' ': c);	
						// str[widthCounter] is either  ' ' (erase is true) or 'c' (erase is false)
					}
					str[heightCounter] = '\0'; // ends char array
					printw(str); // displays str
					move (y+heightCounter,x);
				}
   			}
...
char ReadSelection (string optionName,int & dimension)
{
	...
	else if (optionName=="width" || optionName=="height" || optionName=="side_length")
	{
		...
		//c_str is a member function of string class, it returns a c string .. 
...
int main()
{
	char d;
	WINDOW * wnd;
	int x,y,i; 	// moved i outside of for loop so that it could be referred to when clearing dynamic memory
	int dim;
	char shape;
