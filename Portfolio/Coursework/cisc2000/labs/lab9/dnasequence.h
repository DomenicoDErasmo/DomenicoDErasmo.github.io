...
class DNASequence {
	public:
		...
		friend DNASequence operator-(DNASequence& arg1, const DNASequence& arg2);
		...
		// removed the inline implementation for accessor and mutator functions because it wouldn't compile
		int get_max_sequence() const;
		// Accessor for max_sequence.

		char* get_dna_sequence() const;
		// Accessor for dna_sequence.
		
		// Accessor for validate
		bool get_validate(std::string input);
	
		void change_max_sequence(int input);
		// Mutator for max_sequence

		void change_dna_sequence(std::string input);
